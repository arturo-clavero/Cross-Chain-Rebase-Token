...
contract Vault is ReentrancyGuard, AccessControl, Pausable {
    ...
    function depositCollateral(uint256 amountToDeposit, address token) 
        external 
        nonReentrant 
        whenNotPaused 
    {
    ...
    function borrow(uint256 amountToBorrow, address token, bool takeMaxAvailable) 
        external 
        nonReentrant 
        whenNotPaused
    {
        ...
        uint256 maxAmount = getMaxExtractableLiquidity();
        if (maxAmount == 0)
    ...
    function ethFrom(address _token, uint256 amount) internal view returns (uint256) {
        ...
        if (amount == 0) revert Vault__invalidAmount();
        uint256 converted = PriceConverter.getRates(amount, collateral.priceFeed);
        if (converted == 0) revert Vault__invalidPriceConversion();
        return converted;
    }

    /// @notice Compute the health of available liquidity vs total assets
    ///         0 => unhealthy, WAD => healthy
    ...
    /// @notice Computes the threshold of when an order should be liquidated
    ///         Dynamic value between 0 and WAD depending on liquidityHealthRate
